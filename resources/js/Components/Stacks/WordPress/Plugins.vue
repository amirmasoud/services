      ...
      <li v-for="(plugin, index) in records.plugins" :key="index" class="break-inside-avoid mb-4">
        <div class="flex shadow-sm rounded-md">
          <div class="flex-shrink-0 flex items-start justify-center border-t border-l border-gray-200 w-16 text-white text-sm font-medium rounded-tl-md z-10 bg-white p-2">
            <img :src="plugin.icons.icon_svg || plugin.icons.icon_2x || plugin.icons.icon_1x"  :alt="`${ plugin.name } icon`"/>
          </div>
            ...
            <div class="flex-1 px-4 py-2 text-sm truncate">
              ...
              <span class="flex-grow flex flex-col">
                ...
                <Switch
                  ...
                  @update:modelValue="(plugin) => $emit('update:selected-plugins', selectedPlugins)"
                  :class="[selectedPlugins[plugin.slug] ? 'bg-indigo-600' : 'bg-gray-200', 'relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500']"
                >
                  <span aria-hidden="true" :class="[selectedPlugins[plugin.slug] ? 'translate-x-5' : 'translate-x-0', 'pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200']" />
                </Switch>
              </SwitchGroup>
