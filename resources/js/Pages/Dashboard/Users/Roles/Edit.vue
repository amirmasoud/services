<template>
  <AppHead :title="`Edit ${form.name} role`"/>

  <DashboardMain>
    <template #header>Edit role {{ resource.data.name }}</template>
    <template #buttons></template>
    <template #sub-header></template>
    <template #sub-header-content></template>

    <form @submit.prevent="form.put($route('dashboard.users.roles.update', {role: resource.data.id}))">
...
let form = useForm(`EditRole: ${props.resource.id}`, props.resource.data);
</script>
